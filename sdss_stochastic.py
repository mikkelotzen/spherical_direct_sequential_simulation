...
class SDSS_stochastic():    
    """ Class for performing stochastic spherical direct sequential simulation """
  
    def __init__(self, N_grid = 1000, N_SH = 60, sim_type = "core", sat_height = 350):
    ...
    def attribute_string_update(self):
        ...
        for entry in keys: 
            ...
            if type(dict_obj) is not dict: # avoid printing dicts
    ...
    def grid(self, r_grid, grid_type, calc_sph_d = False):
        ...
        if grid_type == "equal_area":
            while check_flag is False:
                ...
                s_cap, n_regions = sds_util.eq_caps(self.N_grid)
        ...
        if calc_sph_d is True:     
            ...
            self.sph_d = sds_util.haversine(self.r_grid, lon_mesh, lat_mesh, lon_mesh.T, lat_mesh.T)
    ...
    def data(self, *args):
        ...
        A_r, A_theta, A_phi = gt.design_SHA(self.r_grid/self.a*np.ones([self.N_grid]), (90.0-self.lat)*self.rad, self.lon*self.rad, self.N_SH)
        ...
        g = sds_util.gauss_vector(Gauss_in, self.N_SH, i_n = 2, i_m = 3)
