"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
Utility functions for use in SDSSIM
...
def plot_cartopy_global(lat, lon, data=None, unit = "[nT]", cmap = 'PuOr_r', vmin=None, vmax=None, figsize=(8,8), title='Cartopy Earth plot', lat_0 = 0.0, lon_0 = 0.0, point_size=10, showfig=True, norm_class = False, scale_uneven = False, fill = False, savefig = False, dpi = 100, path = None, saveformat = ".png"):
    ...
    if data is None:
        ...
        if scale_uneven == False:
            veven = np.max([np.abs(vmax),np.abs(vmin)])
...
def cartopy_global_plot(lat, lon, data=None, unit = "[nT]", cmap = 'PuOr_r', vmin=None, vmax=None, figsize=(8,8), title='Cartopy Earth plot', lat_0 = 0.0, lon_0 = 0.0, point_size=10, normscale = True, scale_uneven = False, fill = False, savefig = False, dpi = 100, path = None):
    ...
    if savefig is True and path is not None:
        plt.savefig('%s' %path, bbox_inches='tight', dpi = dpi)
    return

def cartopy_global_plot_old(lat, lon, data=None, unit = "[nT]", cmap = 'PuOr_r', vmin=None, vmax=None, figsize=(8,8), title='Cartopy Earth plot', lat_0 = 0.0, lon_0 = 0.0, point_size=10, scale_uneven = False, fill = False, savefig = False, dpi = 100, path = None):
...
def haversine(radius, lon1, lat1, lon2, lat2):
    ...
    b = np.sqrt(a)
    if len(b[np.logical_and(b<1.001,b>1)]) > 0:
        b[b>1] = 1
    c = 2 * np.arcsin(b)
    km = radius * c
    return km

def check_N(N_dict_one, N_dict_two):
    if N_dict_one["N"] != N_dict_two["N"]:
        N_dict_one["N"] = N_dict_two["N"]
        
    return N_dict_one

#def handle_poles(grid_core, data_core, setup_core, grid_sat, data_sat, setup_sat):
